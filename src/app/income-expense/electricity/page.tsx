                    ...
                    <tr key={i} style={{ color: 'white' }}>
                      <td style={{ padding: 8, position: 'relative' }}>
                        ...
                        <div style={{ position: 'absolute', top: '100%', left: 8, minWidth: 160, background: '#ffffff', color: '#111827', borderRadius: 8, boxShadow: '0 14px 35px rgba(0,0,0,0.35)', display: 'none', zIndex: 20 }}>
                          ...
                          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setEditRow(r as { tip: 'GİRİŞ' | 'ÇIKIŞ'; tutar: number; aciklama: string; evrak: string; tarih: string; sekil: 'Nakit' | 'Havale' | 'Kredi Kartı'; durum: 'Ödendi' | 'Ödenmedi' }); (e.currentTarget.parentElement as HTMLDivElement).style.display = 'none'; }} style={{ width: '100%', textAlign: 'left', padding: '8px 12px', border: 'none', background: 'transparent', cursor: 'pointer' }}>✎ Düzenle</button>
