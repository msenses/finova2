...
export default function AccountCreditNewPage() {
  ...
  const [description, setDescription] = useState('Cariyi Alacaklandırma');
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      alert('Cari alacaklandırıldı (demo).');
    ...
    <main style={{ minHeight: '100dvh', color: 'white' }}>
      <div style={{ padding: 16 }}>
        ...
        <section style={{ padding: 16, borderRadius: 12, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.06)', marginBottom: 12 }}>
          <div style={{ fontWeight: 700, marginBottom: 8 }}>Cari Alacaklandır</div>
        ...
        <section style={{ padding: 16, borderRadius: 12, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.06)' }}>
          ...
          <form onSubmit={submit} style={{ display: 'grid', gap: 10, maxWidth: 560 }}>
            ...
            <div>
              ...
              <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Cariyi Alacaklandırma" style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(0,0,0,0.15)', color: 'white' }} />
            </div>
            <div>
            ...
            <div>
              <button disabled={saving} type="submit" style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.2)', background: '#0ea5e9', color: 'white', cursor: 'pointer' }}>
                {saving ? 'Kaydediliyor…' : '🧾 Alacaklandır'}
              </button>
