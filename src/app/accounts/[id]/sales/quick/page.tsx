...
export default function QuickSalePage() {
  ...
  const [groupName, setGroupName] = useState('MÜŞTERİLER');
  const [authorized, setAuthorized] = useState('Ahmet Bey');

  const [date, setDate] = useState(() => new Date().toISOString().slice(0, 10));
  const [paymentType, setPaymentType] = useState('Nakit');
  const [docNo, setDocNo] = useState('');
  const [cash, setCash] = useState('Varsayılan Kasa');
  const [description, setDescription] = useState('Stoksuz Satış');
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!accountId || typeof amount !== 'number' || amount <= 0) {
      setErr('Cari ve tutar zorunlu');
      return;
    }
    setSaving(true);
    try {
      // Basit: stoksuz satış için tek satırsız fatura oluştur
      const { error } = await supabase
        .from('invoices')
        .insert({
          account_id: accountId,
          type: 'sales',
          invoice_no: docNo || null,
          invoice_date: date,
          net_total: amount,
          vat_total: 0,
          total: amount,
      ...
      setErr(e?.message ?? 'Satış yapılamadı');
    } finally {
      setSaving(false);
    }
  };
    ...
    <main style={{ minHeight: '100dvh', color: 'white' }}>
      <div style={{ padding: 16 }}>
        ...
        <section style={{ padding: 16, borderRadius: 12, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.06)' }}>
          <div style={{ fontWeight: 700, marginBottom: 8 }}>Stoksuz Satış Yap</div>
          <form onSubmit={submit} style={{ display: 'grid', gap: 10, maxWidth: 520 }}>
            ...
            <div>
              ...
              <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Stoksuz Satış" style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(0,0,0,0.15)', color: 'white' }} />
            </div>
            <div>
            ...
            <div>
              <div style={{ fontSize: 12, opacity: 0.85, marginBottom: 4 }}>Tahsil Edilen :</div>
              <input type="number" value={received} onChange={(e) => setReceived(e.target.value === '' ? '' : Number(e.target.value))} placeholder="0" style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(0,0,0,0.15)', color: 'white' }} />
            </div>
