...
export default function AccountPaymentNewPage() {
  ...
  const [description, setDescription] = useState('Ödeme');
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      alert('Ödeme kaydı oluşturuldu (demo).');
      router.push((`/accounts/${accountId}`) as Route);
    } catch (e: any) {
      setErr(e?.message ?? 'Ödeme yapılamadı');
      setSaving(false);
    }
  };
    ...
    <main style={{ minHeight: '100dvh', color: 'white' }}>
      <div style={{ padding: 16 }}>
        ...
        <section style={{ padding: 16, borderRadius: 12, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.06)' }}>
          <div style={{ fontWeight: 700, marginBottom: 8 }}>Ödeme Yap</div>
          <form onSubmit={submit} style={{ display: 'grid', gap: 10, maxWidth: 540 }}>
            ...
            <div>
              ...
              <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Ödeme" style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(0,0,0,0.15)', color: 'white' }} />
            </div>
            <div>
            ...
            <div>
              <button disabled={saving} type="submit" style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.2)', background: '#27ae60', color: 'white', cursor: 'pointer' }}>
